...
type Event struct {
	...
	// DeviceID is the v1 compatibility alias used by existing Go callers.
	// It carries the public PDID, not Device.DeviceID.
	DeviceID string `json:"device_id,omitempty"`
	// PDID is the unambiguous public device key for new consumers.
...
func NewEvent(t EventType, pdid string, payload interface{}) Event {
...
// TargetPDID resolves the additive PDID field first and falls back to the
// legacy DeviceID alias. This keeps old producers and new consumers compatible.
func (e Event) TargetPDID() string {
	if e.PDID != "" {
		return e.PDID
	}
	return e.DeviceID
...
// SetTargetPDID normalizes both the explicit field and the v1 compatibility
// alias so relayed events remain readable by old and new clients.
func (e *Event) SetTargetPDID(pdid string) {
	if e == nil || pdid == "" {
		return
	}
	e.PDID = pdid
