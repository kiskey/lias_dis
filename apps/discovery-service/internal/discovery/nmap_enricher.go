...
// Version: 1.2 (Fixed Negative Cache Return & Optimized Flags)
package discovery
...
type NmapEnricher struct {
    ...
    sem    chan struct{} // P2-FIX: Concurrency limiter (max 1 concurrent nmap)
}
...
func (e *NmapEnricher) runNmap(ctx context.Context, ip string) *models.Enrichment {
    ...
    // P1/P2-FIX: Single comprehensive scan with optimized flags
    args := []string{
        "-O",                  // OS detection
        "-sV",                 // Service versioning
        "--version-light",     // Lighter version probes
        "--osscan-limit",      // Only OS scan promising targets
        "--max-retries", "1",  // Reduce probe retries
        "--host-timeout", "5s", // Cap per-host scan duration
    ...
    if err != nil {
        if ctx.Err() == context.DeadlineExceeded {
            ...
            slog.Debug("Nmap execution skipped or unprivileged", "ip", ip, "error", err)
        }
        return nil
    }
