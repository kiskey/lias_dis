...
import (
	...
	"github.com/user/lias-dis/apps/lias/internal/identitymigration"
...
func main() {
	...
	if err != nil {
		...
		overrides, _ := st.LoadDeviceOverrides()
		assignments, _ := st.LoadUserAssignments()
		cache.LoadLocalMetadata(overrides, assignments)
		store = st
	}

	migrationCoordinator := identitymigration.New(store, cache, polEng)
	disClient := liasSync.NewDISClient(cfg.DIS, cache, trigger, broker, migrationCoordinator)
	...
	handlers.SetIdentityGateway(disClient)
	handlers.SetSecureCookies(cfg.HTTP.SecureCookies)
