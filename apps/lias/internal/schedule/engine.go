...
// It implements policy.ScheduleEvaluator.
type Engine struct {
    mu        sync.RWMutex
...
// UpsertSchedule adds or updates a schedule.
...
// EvaluateNow implements the policy.ScheduleEvaluator interface.
// Fails closed (block) if schedule is missing or evaluation errors.
func (e *Engine) EvaluateNow(schedID string) models.Action {
    ...
    if !ok {
        slog.Warn("Schedule not found, failing closed", "schedule_id", schedID)
        return models.ActionBlock
    }

    action, err := Evaluate(s, time.Now())
    if err != nil {
        slog.Error("Schedule evaluation failed, failing closed", "schedule_id", schedID, "error", err)
        return models.ActionBlock
    }
    return action
}

// Run starts the background loop that updates NextStateChange for scheduled devices.
...
// updateNextStateChanges iterates through all devices and updates their
// NextStateChange timestamp if they are governed by a schedule.
