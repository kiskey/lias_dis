...
var (
	...
	ErrUserAssigned = errors.New("user is assigned")
)
...
func (s *Storage) initSchema() error {
    ...
    CREATE TABLE IF NOT EXISTS identity_pdid_redirects (
        old_pdid TEXT PRIMARY KEY,
        new_pdid TEXT NOT NULL
    );

    CREATE TABLE IF NOT EXISTS identity_migration_conflicts (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        old_pdid TEXT NOT NULL,
        new_pdid TEXT NOT NULL,
...
func (s *Storage) ListUsers() ([]models.User, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	rows, err := s.db.Query("SELECT id, name FROM users ORDER BY name, id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Storage) UserAssignmentCount(id string) (int, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	var count int
	err := s.db.QueryRow("SELECT COUNT(*) FROM device_users WHERE user_id = ?", id).Scan(&count)
	return count, err
}
...
func (s *Storage) DeleteUserIfUnassigned(id string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	result, err := s.db.Exec(`DELETE FROM users WHERE id = ?
		AND NOT EXISTS (SELECT 1 FROM device_users WHERE user_id = ?)`, id, id)
	if err != nil {
		return err
	}
	...
	var assigned int
	if err := s.db.QueryRow("SELECT COUNT(*) FROM device_users WHERE user_id = ?", id).Scan(&assigned); err != nil {
		return err
	}
	if assigned > 0 {
...
func (s *Storage) AssignDeviceToUser(pdid, userID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	result, err := s.db.Exec(`INSERT INTO device_users (pdid, user_id)
		SELECT ?, id FROM users WHERE id = ?
		ON CONFLICT(pdid) DO UPDATE SET user_id=excluded.user_id`, pdid, userID)
	if err == nil {
...
func (s *Storage) UnassignDeviceUser(pdid string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	_, err := s.db.Exec("DELETE FROM device_users WHERE pdid = ?", pdid)
	return err
}
...
func (s *Storage) ImportPolicies(jsonData []byte) error {
	...
	for _, p := range policies {
		...
		if err != nil {
			return fmt.Errorf("failed to import policy %q: %w", p.ID, err)
		}
	}
	return tx.Commit()
}
...
func (s *Storage) SaveFlowLog(pdid string, action models.Action, bytes int64) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	// Resolve a reidentified PDID at write time so entries already queued by
	// the asynchronous reporter cannot recreate fragmented source history.
	_, err := s.db.Exec(`INSERT INTO flow_logs (timestamp, pdid, action, bytes)
        VALUES (?, COALESCE((SELECT new_pdid FROM identity_pdid_redirects WHERE old_pdid = ?), ?), ?, ?)`,
		time.Now(), pdid, pdid, string(action), bytes)
	return err
}
